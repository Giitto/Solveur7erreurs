...
public class PanDessin extends JPanel  {
    ...
    protected void imageEclaircie()
    {
		 ...
		 *    1.  A< 1, l image devient plus sombre.
   			  ...
   			  3. K est compris entre 0 et 256 et ajoute un eclairement .
		 */
    ...
    {   // desiner une image a l'ecran
        ...
        //Extraction du raster de monImage (Rappel: monImage de type BufferedImage est compose d'un Raster & d'un ColorModel)
        Raster tramePixel = monImage.getRaster();

        //Extraction du ColorModel
        ColorModel modeleCouleur = monImage.getColorModel();

        //Objet qui sera futur objet de la class color sur lequel on peut appeler les fonctions getRed, getGreen et getBlue qui donne la variation RVB
        Object objCouleur;

        //Ta capter
        int largeur = (int) monImage.getWidth();
        int hauteur = (int) monImage.getHeight();
...
//			System.out.println("");
    ...
    protected int[][][] soustrMat(int[][][] matA, int[][][] matB )
    {
        int tauxErrMax = 5;
        ...
        System.out.println("Taux d'erreur: " + ((((double)compteurErreurPixel/3) / (double)compteurPixel) * 100 ) +  "%" );
